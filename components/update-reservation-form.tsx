...
import { User, Mail, Phone, Calendar, CreditCard, FileText, Building, Users, Baby, Edit, Gift, Percent } from 'lucide-react';
...
import { PriceOffer, UpdateReservationFormData, GuestFormData, SearchParams } from '@/lib/types';

interface UpdateReservationFormProps {
  selectedOffer: PriceOffer;
  searchParams: SearchParams;
  reservationId: number;
  existingData?: Partial<UpdateReservationFormData>;
  onSubmit: (formData: UpdateReservationFormData) => Promise<void>;
  isLoading?: boolean;
}
...
export default function UpdateReservationForm({ 
  ...
  const [formData, setFormData] = useState<UpdateReservationFormData>({
    reservationId,
    contactFirstName: existingData?.contactFirstName || '',
    contactLastName: existingData?.contactLastName || '',
    contactEmail: existingData?.contactEmail || '',
    contactPhone: existingData?.contactPhone || '',
    nationality: existingData?.nationality || 'TR',
    guests: existingData?.guests || Array.from({ length: totalGuests }, (_, index) => ({
    ...
    taxType: existingData?.taxType || 'personal',
    voucherNo: existingData?.voucherNo || '',
    promoCode: existingData?.promoCode || '',
    useGuestBonus: existingData?.useGuestBonus || false,
    isOffer: existingData?.isOffer || false
  });
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <CardContent className="space-y-6">
            {formData.guests.map((guest, index) => (
              <div key={index} className="border rounded-lg p-4">
                ...
                {/* Additional guest fields for update */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  <div>
                    <Label>TC Kimlik No</Label>
                    <Input
                      value={guest.nationalityNo || ''}
                      onChange={(e) => updateGuest(index, 'nationalityNo', e.target.value)}
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => updateGuest(index, 'passportNo', e.target.value)}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="voucherNo">Voucher Numarası</Label>
                <Input
                  id="voucherNo"
                  value={formData.voucherNo || ''}
                  onChange={(e) => setFormData({ ...formData, voucherNo: e.target.value })}
                  placeholder="PNR benzeri voucher numarası"
                />
              </div>
              <div>
                <Label htmlFor="promoCode">Promosyon Kodu</Label>
                <Input
                  id="promoCode"
                  value={formData.promoCode || ''}
                  onChange={(e) => setFormData({ ...formData, promoCode: e.target.value })}
                  placeholder="Promosyon kodu varsa"
                />
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                <Checkbox
                  id="useGuestBonus"
                  checked={formData.useGuestBonus || false}
                  onCheckedChange={(checked) => setFormData({ ...formData, useGuestBonus: checked as boolean })}
                />
                <Label htmlFor="useGuestBonus" className="flex items-center gap-2">
                  <Gift className="h-4 w-4" />
                  Misafir Bonusu Kullan
                </Label>
              </div>
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) => setFormData({ ...formData, isOffer: checked as boolean })}
                />
                <Label htmlFor="isOffer" className="flex items-center gap-2">
        ...
        <div className="flex gap-4">
          ...
          <Button
            ...
            className="flex-1"
          >
            {isLoading ? 'Güncelleniyor...' : 'Rezervasyonu Güncelle'}
          </Button>
        </div>
      </form>
    </div>
